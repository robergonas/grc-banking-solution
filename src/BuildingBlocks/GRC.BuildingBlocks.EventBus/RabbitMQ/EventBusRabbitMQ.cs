using GRC.BuildingBlocks.EventBus.Abstractions;
using GRC.BuildingBlocks.EventBus.Events;
using GRC.BuildingBlocks.EventBus.Extensions;
...
{
    const string BROKER_NAME = "grc_event_bus";
        ...
        if (_subsManager.HasSubscriptionsForEvent(eventName))
        {
            ...
            foreach (var subscription in subscriptions)
            {
                if (subscription.IsDynamic)
                {
                    ...
                    using dynamic eventData = JsonConvert.DeserializeObject<dynamic>(message);
                    await Task.Yield();
